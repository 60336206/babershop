...
import { of } from 'rxjs';
import { apiservicedelete } from './apiservicedelete';

describe('apiservicedelete', () => {
  it('should call the function and return response', (done) => {
    ...
    apiservicedelete(httpClientMock, 'http://localhost', {
        body: {
          idAppointment: '1',
          idCustomer: '1',
          idService: '1',
  ...
  it('should handle undefined params', (done) => {
    ...
    try {
      apiservicedelete(httpClientMock, 'http://localhost', undefined as any).subscribe(() => done());
    } catch(e) {
      done();
    }
  });
