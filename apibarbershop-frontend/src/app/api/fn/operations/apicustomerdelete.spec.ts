...
import { of } from 'rxjs';
import { apicustomerdelete } from './apicustomerdelete';

describe('apicustomerdelete', () => {
  it('should call the function and return response', (done) => {
    ...
    apicustomerdelete(httpClientMock, 'http://localhost', {
        body: {
          idAppointment: '1',
          idCustomer: '1',
          idService: '1',
  ...
  it('should handle undefined params', (done) => {
    ...
    try {
      apicustomerdelete(httpClientMock, 'http://localhost', undefined as any).subscribe(() => done());
    } catch(e) {
      done();
    }
  });
