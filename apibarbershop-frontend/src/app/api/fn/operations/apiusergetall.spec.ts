...
import { of } from 'rxjs';
import { apiusergetall } from './apiusergetall';

describe('apiusergetall', () => {
  it('should call the function and return response', (done) => {
    ...
    apiusergetall(httpClientMock, 'http://localhost', {
        body: {
          idAppointment: '1',
          idCustomer: '1',
          idService: '1',
  ...
  it('should handle undefined params', (done) => {
    ...
    try {
      apiusergetall(httpClientMock, 'http://localhost', undefined as any).subscribe(() => done());
    } catch(e) {
      done();
    }
  });
