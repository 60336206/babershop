...
import { of } from 'rxjs';
import { apiauthlogin } from './apiauthlogin';
...
describe('apiauthlogin', () => {
  it('should call the function and return response', (done) => {
    ...
    apiauthlogin(httpClientMock, 'http://localhost', {
        body: {
          idAppointment: '1',
          idCustomer: '1',
          idService: '1',
  ...
  it('should handle undefined params', (done) => {
    ...
    try {
      apiauthlogin(httpClientMock, 'http://localhost', undefined as any).subscribe(() => done());
    } catch(e) {
      done();
    }
  });
