...
import { of } from 'rxjs';
import { apiserviceupdate } from './apiserviceupdate';

describe('apiserviceupdate', () => {
  it('should call the function and return response', (done) => {
    ...
    apiserviceupdate(httpClientMock, 'http://localhost', {
        body: {
          idAppointment: '1',
          idCustomer: '1',
          idService: '1',
  ...
  it('should handle undefined params', (done) => {
    ...
    try {
      apiserviceupdate(httpClientMock, 'http://localhost', undefined as any).subscribe(() => done());
    } catch(e) {
      done();
    }
  });
