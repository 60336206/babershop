...
import { of } from 'rxjs';
import { apicustomerupdate } from './apicustomerupdate';

describe('apicustomerupdate', () => {
  it('should call the function and return response', (done) => {
    ...
    apicustomerupdate(httpClientMock, 'http://localhost', {
        body: {
          idAppointment: '1',
          idCustomer: '1',
          idService: '1',
  ...
  it('should handle undefined params', (done) => {
    ...
    try {
      apicustomerupdate(httpClientMock, 'http://localhost', undefined as any).subscribe(() => done());
    } catch(e) {
      done();
    }
  });
