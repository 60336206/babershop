...
import { of } from 'rxjs';
import { apicustomerinsert } from './apicustomerinsert';

describe('apicustomerinsert', () => {
  it('should call the function and return response', (done) => {
    ...
    apicustomerinsert(httpClientMock, 'http://localhost', {
        body: {
          idAppointment: '1',
          idCustomer: '1',
          idService: '1',
  ...
  it('should handle undefined params', (done) => {
    ...
    try {
      apicustomerinsert(httpClientMock, 'http://localhost', undefined as any).subscribe(() => done());
    } catch(e) {
      done();
    }
  });
