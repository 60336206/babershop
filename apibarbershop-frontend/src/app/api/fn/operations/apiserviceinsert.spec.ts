...
import { of } from 'rxjs';
import { apiserviceinsert } from './apiserviceinsert';

describe('apiserviceinsert', () => {
  it('should call the function and return response', (done) => {
    ...
    apiserviceinsert(httpClientMock, 'http://localhost', {
        body: {
          idAppointment: '1',
          idCustomer: '1',
          idService: '1',
  ...
  it('should handle undefined params', (done) => {
    ...
    try {
      apiserviceinsert(httpClientMock, 'http://localhost', undefined as any).subscribe(() => done());
    } catch(e) {
      done();
    }
  });
