...
import { of } from 'rxjs';
import { apiservicegetall } from './apiservicegetall';

describe('apiservicegetall', () => {
  it('should call the function and return response', (done) => {
    ...
    apiservicegetall(httpClientMock, 'http://localhost', {
        body: {
          idAppointment: '1',
          idCustomer: '1',
          idService: '1',
  ...
  it('should handle undefined params', (done) => {
    ...
    try {
      apiservicegetall(httpClientMock, 'http://localhost', undefined as any).subscribe(() => done());
    } catch(e) {
      done();
    }
  });
