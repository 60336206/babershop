...
import { of } from 'rxjs';
import { apisettinggetone } from './apisettinggetone';

describe('apisettinggetone', () => {
  it('should call the function and return response', (done) => {
    ...
    apisettinggetone(httpClientMock, 'http://localhost', {
        body: {
          idAppointment: '1',
          idCustomer: '1',
          idService: '1',
  ...
  it('should handle undefined params', (done) => {
    ...
    try {
      apisettinggetone(httpClientMock, 'http://localhost', undefined as any).subscribe(() => done());
    } catch(e) {
      done();
    }
  });
