...
import { of } from 'rxjs';
import { apiappointmentuploadimage } from './apiappointmentuploadimage';

describe('apiappointmentuploadimage', () => {
  it('should call the function and return response', (done) => {
    ...
    apiappointmentuploadimage(httpClientMock, 'http://localhost', {
        body: {
          idAppointment: '1',
          idCustomer: '1',
          idService: '1',
  ...
  it('should handle undefined params', (done) => {
    ...
    try {
      apiappointmentuploadimage(httpClientMock, 'http://localhost', undefined as any).subscribe(() => done());
    } catch(e) {
      done();
    }
  });
