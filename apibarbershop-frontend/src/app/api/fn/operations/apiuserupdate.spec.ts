...
import { of } from 'rxjs';
import { apiuserupdate } from './apiuserupdate';

describe('apiuserupdate', () => {
  it('should call the function and return response', (done) => {
    ...
    apiuserupdate(httpClientMock, 'http://localhost', {
        body: {
          idAppointment: '1',
          idCustomer: '1',
          idService: '1',
  ...
  it('should handle undefined params', (done) => {
    ...
    try {
      apiuserupdate(httpClientMock, 'http://localhost', undefined as any).subscribe(() => done());
    } catch(e) {
      done();
    }
  });
