...
import { of } from 'rxjs';
import { apiuserinsert } from './apiuserinsert';

describe('apiuserinsert', () => {
  it('should call the function and return response', (done) => {
    ...
    apiuserinsert(httpClientMock, 'http://localhost', {
        body: {
          idAppointment: '1',
          idCustomer: '1',
          idService: '1',
  ...
  it('should handle undefined params', (done) => {
    ...
    try {
      apiuserinsert(httpClientMock, 'http://localhost', undefined as any).subscribe(() => done());
    } catch(e) {
      done();
    }
  });
