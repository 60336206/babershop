...
import { of } from 'rxjs';
import { apigallerygetall } from './apigallerygetall';

describe('apigallerygetall', () => {
  it('should call the function and return response', (done) => {
    ...
    apigallerygetall(httpClientMock, 'http://localhost', {
        body: {
          idAppointment: '1',
          idCustomer: '1',
          idService: '1',
  ...
  it('should handle undefined params', (done) => {
    ...
    try {
      apigallerygetall(httpClientMock, 'http://localhost', undefined as any).subscribe(() => done());
    } catch(e) {
      done();
    }
  });
