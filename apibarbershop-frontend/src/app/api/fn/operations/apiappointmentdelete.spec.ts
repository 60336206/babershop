...
import { of } from 'rxjs';
import { apiappointmentdelete } from './apiappointmentdelete';

describe('apiappointmentdelete', () => {
  it('should call the function and return response', (done) => {
    ...
    apiappointmentdelete(httpClientMock, 'http://localhost', {
        body: {
          idAppointment: '1',
          idCustomer: '1',
          idService: '1',
  ...
  it('should handle undefined params', (done) => {
    ...
    try {
      apiappointmentdelete(httpClientMock, 'http://localhost', undefined as any).subscribe(() => done());
    } catch(e) {
      done();
    }
  });
