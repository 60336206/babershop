...
import { of } from 'rxjs';
import { apiappointmentgetfiles } from './apiappointmentgetfiles';

describe('apiappointmentgetfiles', () => {
  it('should call the function and return response', (done) => {
    ...
    apiappointmentgetfiles(httpClientMock, 'http://localhost', {
        body: {
          idAppointment: '1',
          idCustomer: '1',
          idService: '1',
  ...
  it('should handle undefined params', (done) => {
    ...
    try {
      apiappointmentgetfiles(httpClientMock, 'http://localhost', undefined as any).subscribe(() => done());
    } catch(e) {
      done();
    }
  });
