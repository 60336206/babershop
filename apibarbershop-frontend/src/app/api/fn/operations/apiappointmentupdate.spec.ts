...
import { of } from 'rxjs';
import { apiappointmentupdate } from './apiappointmentupdate';

describe('apiappointmentupdate', () => {
  it('should call the function and return response', (done) => {
    ...
    apiappointmentupdate(httpClientMock, 'http://localhost', {
        body: {
          idAppointment: '1',
          idCustomer: '1',
          idService: '1',
  ...
  it('should handle undefined params', (done) => {
    ...
    try {
      apiappointmentupdate(httpClientMock, 'http://localhost', undefined as any).subscribe(() => done());
    } catch(e) {
      done();
    }
  });
