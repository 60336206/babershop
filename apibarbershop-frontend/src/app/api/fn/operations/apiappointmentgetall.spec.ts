...
import { of } from 'rxjs';
import { apiappointmentgetall } from './apiappointmentgetall';

describe('apiappointmentgetall', () => {
  it('should call the function and return response', (done) => {
    ...
    apiappointmentgetall(httpClientMock, 'http://localhost', {
        body: {
          idAppointment: '1',
          idCustomer: '1',
          idService: '1',
  ...
  it('should handle undefined params', (done) => {
    ...
    try {
      apiappointmentgetall(httpClientMock, 'http://localhost', undefined as any).subscribe(() => done());
    } catch(e) {
      done();
    }
  });
