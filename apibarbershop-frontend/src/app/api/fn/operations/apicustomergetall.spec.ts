...
import { of } from 'rxjs';
import { apicustomergetall } from './apicustomergetall';

describe('apicustomergetall', () => {
  it('should call the function and return response', (done) => {
    ...
    apicustomergetall(httpClientMock, 'http://localhost', {
        body: {
          idAppointment: '1',
          idCustomer: '1',
          idService: '1',
  ...
  it('should handle undefined params', (done) => {
    ...
    try {
      apicustomergetall(httpClientMock, 'http://localhost', undefined as any).subscribe(() => done());
    } catch(e) {
      done();
    }
  });
